...
namespace ezvm.Core.VM
{
    ...
    public class VirtualMachine
    {
        ...
        public event EventHandler OnOperationExecute;
        public event EventHandler OnOperationExecuted;
        public event EventHandler OnMethodEnter;
        public event EventHandler OnMethodEntered;
        public event EventHandler OnMethodExit;
        public event EventHandler OnMethodExited;
        ...
        public void Step(bool ignoreErrors = false)
        {
            try
            {
                ...
                OnOperationExecute?.Invoke(this, EventArgs.Empty);
                if (opData.OpCode == OperationCode.Call)
                    OnMethodEnter?.Invoke(this, EventArgs.Empty);
                else if (opData.OpCode == OperationCode.Return)
                    OnMethodExit?.Invoke(this, EventArgs.Empty);

                op.Process(this, opData);

                OnOperationExecuted?.Invoke(this, EventArgs.Empty);
                if (opData.OpCode == OperationCode.Call)
                    OnMethodEntered?.Invoke(this, EventArgs.Empty);
                else if (opData.OpCode == OperationCode.Return)
                    OnMethodExited?.Invoke(this, EventArgs.Empty);
            }
            catch (Exception ex)
            {
